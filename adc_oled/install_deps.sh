#!/bin/bash
#==============================================================================
# install_adc_oled_deps.sh - Instalar dependencias para proyecto ADC+OLED
#==============================================================================
# Uso: sudo bash install_adc_oled_deps.sh
#==============================================================================
...
print_header() {
    echo ""
    echo -e "${BLUE}╔══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║     INSTALADOR - ADC + OLED + KY-037 - RASPBERRY PI     ║${NC}"
    echo -e "${BLUE}╚══════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
show_instructions() {
    ...
    echo -e "  ${YELLOW}cd /mnt/disk/src/rpico/pico_src/adc_oled${NC}"
    echo -e "  ${YELLOW}bash flash_adc_oled.sh${NC}"
    echo ""
    echo -e "Para solo compilar:"
    echo -e "  ${YELLOW}bash flash_adc_oled.sh --compile-only${NC}"
    echo ""
    echo -e "Para solo flashear:"
    echo -e "  ${YELLOW}bash flash_adc_oled.sh --flash-only${NC}"
    echo ""
}
...
echo ""
print_step "Script de flasheo listo: flash_adc_oled.sh"
echo ""
